...
import pipelines.examples.data.{ MlAction, ModelDescriptor, ModelDescriptorUtil }
...
object ModelToServe {
  ...
  def fromModelDescriptor(m: ModelDescriptor): ModelToServe = {
    ModelToServe(m.name, m.description, ModelDescriptorUtil.getModelValue(m.modeltype), m.data.getBytes, null, m.dataType)
  }
  ...
  def writeModel[RECORD, RESULT](model: Model[RECORD, RESULT], output: DataOutputStream): Unit = {
    try {
      ...
      output.writeLong(ModelDescriptorUtil.getModelValue(model.getType))
      output.write(bytes)
    } catch {
      case t: Throwable ⇒
  ...
  //      case Some(model) ⇒
  //        validateResolver()
  //        Some(resolver.getFactory(ModelDescriptorUtil.getModelValue(model.getType).get.restore(model.toBytes()).asInstanceOf[Model[RECORD, RESULT]])
  //      case _ ⇒ None
  //    }
  //  }
