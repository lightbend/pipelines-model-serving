...
object Logger {
  ...
  /**
   * Always call this method to create the actual logger instance, wrapped in a [[MutableLogger]].
   * To override in a test to use stdout, call `mylogger.setLogger(StdoutStderrLogger)`.
   */
  def make[T](clazz: Class[T]): MutableLogger = {
...
trait Logger {
  ...
  def throwable(msg: String, th: Throwable): Unit = {
    error(s"$msg. Failed with exception: $th")
    LoggingUtil.throwableToStrings(th).foreach(error)
  }
}
