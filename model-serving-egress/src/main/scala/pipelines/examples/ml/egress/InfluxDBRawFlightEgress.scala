...
object InfluxDBRawFlightEgress extends FlowEgress[AirlineFlightRecord] {
  ...
  override def createLogic = new FlowEgressLogic() {
    ...
    def flow = flowWithPipelinesContext()
      .map { record ⇒
        {
          println(s"InfluxDBRawEgress: record of type = AirlineFlightResult")
          InfluxDBUtil.write(record, "flight_record", streamletRefConfig.getString(influxDatabase), influxDB)
          record
        }
      }
  }
