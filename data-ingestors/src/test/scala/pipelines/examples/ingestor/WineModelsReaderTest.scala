...
class WineModelsReaderTest extends FunSpec with BeforeAndAfter with OutputInterceptor {
  ...
  val testGoodModelsResources = Array("100_winequality_red.csv")
  val testBadModelsResources = Array("error_winequality_red.csv")

  describe("WineModelsReader") {
    it("Loads one or more model file resources from the classpath") {
      expectOutput() {
        assert(WineModelsReader(WineModelDataIngress.WineModelsResources).next() != null)
      }
    }
